  ...
  return (
    <div className="space-y-8 relative">
                ...
                <div className="space-y-4">
                   <div className="grid grid-cols-2 gap-4">
                      <div>
                         ...
                         <input required type="text" value={newPlayer.firstName} onChange={e => setNewPlayer({ ...newPlayer, firstName: e.target.value })} className="w-full bg-slate-50 border rounded-2xl px-5 py-4 text-xs font-bold" />
                      </div>
                      <div>
                         <label className="text-[10px] font-black text-slate-400 uppercase">Apellidos</label>
                         <input required type="text" value={newPlayer.lastName} onChange={e => setNewPlayer({ ...newPlayer, lastName: e.target.value })} className="w-full bg-slate-50 border rounded-2xl px-5 py-4 text-xs font-bold" />
                      </div>
                   </div>
                   <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-[10px] font-black text-slate-400 uppercase">DNI</label>
                        <input required type="text" value={newPlayer.idCard} onChange={e => setNewPlayer({ ...newPlayer, idCard: e.target.value })} className="w-full bg-slate-50 border rounded-2xl px-5 py-4 text-xs font-bold" />
                      </div>
                      <div>
                        <label className="text-[10px] font-black text-slate-400 uppercase">Camiseta #</label>
                        <input required type="number" value={newPlayer.number} onChange={e => setNewPlayer({ ...newPlayer, number: e.target.value })} className="w-full bg-slate-50 border rounded-2xl px-5 py-4 text-xs font-bold" />
                      </div>
                   </div>
                </div>
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b pb-6">
        <div>
          <h1 className="text-4xl font-black text-slate-900 italic uppercase">Sección del Club</h1>
            ...
            <div className="space-y-4">
              {players.length === 0 ? (
                ...
                players.sort((a,b) => a.number - b.number).map((p) => (
                  <div key={p.id} className="group flex items-center justify-between p-6 rounded-[28px] border bg-white border-slate-100 transition-all">
                    <div className="flex items-center gap-6">
                      <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-sm font-black bg-slate-900 text-white">
                        {p.number}
                      </div>
                      <div>
                        ...
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">DNI: {p.idCard}</p>
                      </div>
                    </div>
                    
                    <div className="flex items-center gap-3">
                       <div className="px-5 py-2 rounded-xl text-[10px] font-black uppercase bg-slate-100 text-slate-500">
                         {p.number <= 15 ? 'Titular' : 'Suplente'}
                       </div>
                    </div>
                  </div>
                ))
